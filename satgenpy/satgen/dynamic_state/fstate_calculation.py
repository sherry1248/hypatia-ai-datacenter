...
def calculate_alternative_path_candidates(
        routing_graph,
        source,
        destination,
        max_paths=3
    ...
    """
    Find up to max_paths loop-free alternative paths between two satellites.

    Paths are ordered by total edge weight.
    """
    ...
    try:
        ...
        for rank, path in enumerate(paths, start=1):
            ...
            for u, v in zip(path[:-1], path[1:]):
                total_distance_m += routing_graph.edges[(u, v)]["weight"]
